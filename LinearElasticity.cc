...
LinearElasticity::LinearElasticity(TopOpt *opt){
        ...
        // Set up stiffness matrix, load vector and boundary conditions (Dirichlet) for the design problem
        SetUpLoadAndBC(opt);
}
...
PetscErrorCode LinearElasticity::DMDAGetElements_2D(DM dm,PetscInt *nel,PetscInt *nen,const PetscInt *e[]) {
	PetscErrorCode ierr;
...
PetscInt LinearElasticity::Quad4Isoparametric(PetscScalar *X, PetscScalar *Y, PetscScalar nu, PetscInt AssType, PetscScalar *ke){
	// 
	...
	for (PetscInt ii=0; ii<2; ii++){
		for (PetscInt jj=0; jj<2; jj++){
			...
			for (PetscInt i=0; i<4; i++){
				B[0][2*i]   = invJ[0][0]*dNdxi[i]+invJ[0][1]*dNdeta[i];
				B[1][2*i+1] = invJ[1][0]*dNdxi[i]+invJ[1][1]*dNdeta[i];
				B[2][2*i]   = invJ[1][0]*dNdxi[i]+invJ[1][1]*dNdeta[i];
				B[2][2*i+1] = invJ[0][0]*dNdxi[i]+invJ[0][1]*dNdeta[i];
			}
...
void LinearElasticity::DifferentiatedShapeFunctions(PetscScalar xi, PetscScalar eta, PetscScalar *dNdxi, PetscScalar *dNdeta){
...
PetscScalar LinearElasticity::Inverse2M(PetscScalar J[][2], PetscScalar invJ[][2]){
