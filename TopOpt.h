...
public:
	...
	// Periodisation
	PetscErrorCode PeriodizationMatrix(TopOpt *opt);

	// Physical domain variables
	PetscScalar xc[4]; // Domain coordinates 
	PetscScalar dx,dy,dz; // Element size
	PetscInt nxyz[2]; // Number of nodes in each direction
	...
	PetscInt divx; // Number of divisions in x
	PetscInt divy; // Number of divisions in y
	Mat DT; // Mapping matrix
	PetscInt MapType;
	...
	Vec xo1, xo2, U, L;
        PetscScalar  F0;
        PetscInt AssType; // Plane assumption; 0: Plane Strain;  1: Plane stress;
