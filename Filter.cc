...
PetscErrorCode Filter::FilterProject(TopOpt *opt){
	...
	// COPY IN CASE OF SENSITIVITY FILTER
            ...
            ierr = VecCopy(opt->x,opt->xPhys); CHKERRQ(ierr); 
        }
...
PetscErrorCode Filter::SetUp(TopOpt *opt){
	...
	if (opt->filter==0 || opt->filter==1){
		...
		ierr = DMDAGetInfo(opt->da_nodes,NULL,&M,&N,NULL,&md,&nd,NULL,NULL,NULL,&bx,&by,NULL,&stype); CHKERRQ(ierr);
		...
		ElemConn = (PetscInt)PetscMax(ceil(opt->rmin/dx)-1,PetscMax(ceil(opt->rmin/dy)-1, 0));
		...
		DMDAGetOwnershipRanges(opt->da_nodes, &LxCorrect, &LyCorrect,NULL); 
		...
		DMDACreate2d(PETSC_COMM_WORLD,bx,by,stype,M-1,N-1,md,nd,
				1,ElemConn,Lx,Ly,&da_elem);
		// Initialize
		...
		// 1. Run through all elements in the mesh - should not include ghosts
				...
				for (PetscInt i=info.xs; i<info.xs+info.xm; i++) {
					PetscInt col = (i-info.gxs) + (j-info.gys)*(info.gxm) ;
					...
					for (PetscInt j2=std::max(j-info.sw,0);j2<=std::min(j+info.sw,info.my);j2++){
						for (PetscInt i2=std::max(i-info.sw,0);i2<=std::min(i+info.sw,info.mx);i2++){
							PetscInt row = (i2-info.gxs) + (j2-info.gys)*(info.gxm);
							PetscScalar dist = 0.0;
							for(PetscInt kk=0; kk<2; kk++){
								dist = dist + PetscPowScalar(lcoorp[2*col+kk]-lcoorp[2*row+kk],2.0);
							}
