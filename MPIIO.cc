...
MPIIO::MPIIO(DM da_nodes, int nPf, std::string pnames,int nCf, std::string cnames){
	...
	nCellsMyrank[0] = nel; // We have this number from when we called DMDAGetElements_2D
	...
	float *pointsDomain0 = new float[2*nPointsMyrank[0]];
	for (unsigned long int i=0; i<2*nPointsMyrank[0]; i++){
	...
	for (unsigned long int i=0;i<nCellsMyrank[0];i++){
		...
		if (nen == 4){ // Quad element
			cellsTypes0[i] = 9; // (in vtk, quad element type is 9)
		}
		...
		if (nen == 4){ // Quad element
			CellOffset+=nen;
                        cellsOffset0[i] = CellOffset; // (in vtk hex element type is 9)
                } 
...
PetscErrorCode MPIIO::WriteVTK(DM da_nodes, Vec U, TopOpt *opt, PetscInt itr){
  ...
  // Here we only have one "timestep" (no optimization)
	...
	for (unsigned long int i=0; i<nPointsMyrank[0]; i++){
		// Ux
		workPointField[i] = float(UlocalPointer[2*i]);
		// Uy
		workPointField[i+nPointsMyrank[0]] = float(UlocalPointer[2*i+1]);
	}
...
PetscErrorCode MPIIO::DMDAGetElements_2D(DM dm,PetscInt *nel,PetscInt *nen,const PetscInt *e[]) {
    PetscErrorCode ierr;
